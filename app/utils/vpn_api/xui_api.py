...
if not config.xui_config.xui_use_ssl_cert:
...
class XUIApi:
    ...
    def __init__(self):
        """Инициализация API"""
        self.api = py3xui.Api(
            host=config.xui_config.xui_host,
            username=config.xui_config.xui_username,
            password=config.xui_config.xui_password,
        )
        self._login()
    ...
    def _login(self):
        ...
        try:
            self.api.login()
            logger.info(f"Successfully logged in to 3x-ui at {config.xui_config.xui_host}")
    ...
    def _build_config_url(self, inbound: Inbound, client: Client) -> str:
        ...
        # Базовый URL
        base_url = f"vless://{client.id}@{config.xui_config.domain}:{inbound.port}"
    ...
    def get_subscription_url(self, email: str) -> str:
        ...
        return f"https://{config.xui_config.domain}:{config.xui_config.subscription_port}/{email}"
